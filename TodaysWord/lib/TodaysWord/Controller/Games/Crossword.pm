package TodaysWord::Controller::Games::Crossword;
use Moose;
...
TodaysWord::Controller::Games::Crossword - Catalyst Controller
...
sub index :Path :Args(0) {
    ...
    $c->stash( template => 'games/crossword/crosswords.tt', crosswords => [$c->model('DB')->resultset('Crossword')->all]);
}
...
sub play :Local {
    ...
    $c->stash( template => 'games/crossword/crossword.tt', crossword => $crossword, cells => $cells,
                down_clues => $down_clues, across_clues => $across_clues, game_play => $game_play, timer_start => $timer_start);
}
...
sub pause :Local {
    ...
    $c->stash( template => 'games/crossword/pause_crossword.tt', game_play => $game_play);
}
...
sub resume :Local {
    ...
    $c->stash( template => 'games/crossword/resume_crossword.tt');
}
...
sub done :Local {
    ...
    # Get the answer key
    my ($crossword, $cells, $down_clues, $across_clues) = TodaysWord::Model::Crossword->get_crossword_data($c, $game_play->game_id);

    my %AnswerBoard;
    foreach my $cell(@{$cells}) {
       $AnswerBoard{$cell->y}{$cell->x}->{'solution'} = $cell->solution;
       $AnswerBoard{$cell->y}{$cell->x}->{'number'} = $cell->number;
    }
    ...
    foreach $y(sort{$a <=> $b} keys %AnswerBoard) {
       foreach my $x(sort{$a <=> $b} keys %{$AnswerBoard{$y}}) {
          ...
          $result{'number'} = $AnswerBoard{$y}{$x}->{'number'};
    ...
    $c->stash( template => 'games/crossword/done_crossword.tt', crossword => $crossword, cells => \@ResultCells,
                down_clues => $down_clues, across_clues => $across_clues, score => $score, time_played => $time_played);
}
