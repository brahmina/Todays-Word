package TodaysWord::Controller::Games::TodaysWord;
use Moose;
...
TodaysWord::Controller::Games::TodaysWord - Catalyst Controller
...
sub index :Path :Args(0) {
    ...
    $c->stash( template => 'games/todaysword/todayswords.tt', todayswords => [$c->model('DB')->resultset('TodaysWord')->all]);
}
...
sub play :Local {
    ...
    $c->stash( template => 'games/todaysword/todaysword.tt', todaysword => $todaysword, game_play => $game_play, timer_start => $timer_start);
}
...
sub pause :Local {
    ...
    $c->stash( template => 'games/todaysword/pause_todaysword.tt', game_play => $game_play);
}
...
sub resume :Local {
    ...
    $c->stash( template => 'games/todaysword/resume_todaysword.tt');
}
...
sub done :Local {
    ...
    # Get the answer key
    my $todaysword = $c->model('DB::TodaysWord')->find({id => $game_play->game_id});
    my $cells = $c->model('DB::TodaysWordCell')->find({todaysword_id => $todaysword->id});
    ...
    $c->stash( template => 'games/todaysword/done_todaysword.tt', todaysword => $todaysword, score => $score, time_played => $time_played);
}
