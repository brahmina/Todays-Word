...
// 
// Edited by Marilyn to make it work in list central December 12th 2009
var DatePicker = new Class({
    ...
    attach: function () {
        ...
        $$(this.attachTo).each(function (item, index) {
            ...
            if(this.options.inputID){
                $(this.options.inputID).value = init_clone_val;
            }
            var display = item.getStyle('display');
            var clone = item.setStyle('display', this.options.debug ? display : 'none').store('datepicker', true).clone().store('datepicker', true).removeProperty('name').setStyle('display', display).set('value', init_clone_val).inject(item, 'after');
            if ($chk(this.options.toggleElements)) {
                togglers[index].setStyle('cursor', 'pointer').addEvents({
                    'click': function (e) {
    ...
    fx: function (fx) {
        if (fx == 'right') {
            ...
            this.oldContents.setStyle('left', 0).set('tween', {
    ...
    renderTime: function () {
        ...
        new Element('input', {
            type: 'text',
            'class': 'hour'
        }).set('value', this.leadZero(this.d.getHours())).addEvents({
        ...
        }).set('maxlength', 2).inject(container);
        new Element('input', {
            type: 'text',
            'class': 'minutes'
        }).set('value', this.leadZero(this.d.getMinutes())).addEvents({
            mousewheel: function (e) {
                var i = e.target,
                    v = i.get('value').toInt();
                i.focus();
    ...
    select: function (values) {
        ...
        if(this.options.inputID){
            $(this.options.inputID).value = this.visual.value;
        }
    ...
    unformat: function (t, format) {
        ...
        for (c in a) {
            ...
            case 'n':
                ...
                break;
            case 'M':
                v = this.options.months.filter(function (item, index) {
                    return item.substring(0, this.options.monthShort) == v
                }.bind(this))[0];
            case 'F':
